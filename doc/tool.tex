...
\ifx\standalonemanual1\else
  \section{The proof assistant}
\fi
\label{sec:tool}

\ifx\standalonemanual1 This is \else This section constitutes \fi a user manual for our proof assistant for
...
\ifanonymous
\showafter{2018}{07}{20}{\POPLspecific}
A link to the binaries and the sources is included in the nonanonymous supplement.
\else
The source code is published on GitHub \cite{github-source}\ifx\standalonemanual1\else, 
a binary distribution is available in \cite{qrhl-binary}\fi.
\fi
...
defined \ifx\standalonemanual1 in \cite{qrhl-paper-from-manual}\else in this paper\fi. These can be declared using the following commands
\begin{center}
...
\begin{longtable}{|p{.275\hsize}|>{\parskip=\medskipamount}p{.66\hsize}|}
  ...
  \ruleref{QInit1}).\footnote{\label{footnote:mkUnit}Formally, changing the type of programs
    ...
    \ifx\standalonemanual1 in \cite{qrhl-paper-from-manual}\else in this paper\fi.
    %
...
%  \symbolindexmark\TOOLH\symbolindexmark\TOOLX\symbolindexmark\TOOLY\symbolindexmark\TOOLZ
  ...
  and $\boundedv W$
  described in the preliminaries\ifx\standalonemanual1 of
  \cite{qrhl-paper-from-manual}\fi. That is,
  $A\otimes B\neq B\otimes A$.)
  ...
  \\
  \hline \subhead{States} \hline \constdef{$\basis{}x$\par
    $\mathtt{ket}\ x$}
  ...
  \TODOQ{put back when lift is in arxiv}
  ...
  % (See \qrhlautorefx{def:lift}.)
  \TODOQ{Put back when present in arxiv}
...
%
  ...
  \texinput{\textnormal{There is no input method support for entering $\mathfrak{Cla}$.
      Use the $\mathtt{Cla}[\dots]$ syntax instead.}}
  \\
  \hline
  ...
  \texinput{\textnormal{There is no input method support for
      $\mathfrak q$.
      Use, e.g., the \texttt{==q} syntax instead of
      $\mathord\equiv\mathfrak q$.}}
  \\
  %
  \hline
...
\texttt{rnd.qrhl}\nonanonymous{\footnote{\giturl{rnd.qrhl}, and bundled with the tool.}}) which does not seem easily possible in EasyCrypt.
...
\texttt{QRHL} provides numerous definitions (most of them listed in
...
\begin{tabular}{|ll|}
  ...
  &
    \bfseries Lemma in \ifx\standalonemanual1 \cite{qrhl-paper-from-manual} \else this paper \fi
  \\
  \hline
  \hline
...
\paragraph{Code generation.} If all quantum variables involved in a
...
can be found in \texttt{Code\_Example.thy}.\footnote{Bundled with the
  tool\nonanonymous{, and also directly available at \giturl{Code\_Example.thy}}.}  (A
...
\begin{definition}[ROR-OT-CPA advantage]\label{def:roradv}
  ...
  holds after running the instructions in $G$,
  and $\eta$
...
\texttt{msg} as abstract (i.e., unspecified) types:
\begin{lstlisting}
  typedecl key
  typedecl msg
\end{lstlisting}
Furthermore, we need to declare those types as finite (otherwise
uniform sampling of keys/messages is not well-defined). We use
Isabelle's type class mechanism for this:
\begin{lstlisting}
  instantiation key :: finite [...]
  instantiation msg :: finite [...]
\end{lstlisting}
Since we cannot prove that those types are finite (they are declared
abstractly, after all), the proof obligations from those
\texttt{instantiation} commands are discharged with \texttt{sorry}
(which tells Isabelle to simply believe the claim).

We also need to declare that there is an operation $+$
(the XOR operation) on \texttt{msg} that is a group operation, and
that satisfies the rule $a+a=0$.
This is done by declaring \texttt{msg} as an additive group via the type class mechanism and adding an axiom that specifies the cancellation property:
\begin{lstlisting}
...
\texttt{prg-enc-rorcpa.qrhl}.\footnote{Bundled with the tool\nonanonymous{, and also
...
\end{multline*}
At the first glance, it would seem that the right thing to do is to
...
In our tool,\footnote{File \texttt{prg-enc-indcpa.qrhl}, bundled with
  the tool\nonanonymous{, and also directly available here: \giturl{prg-enc-indcpa.qrhl}}.} we have
...
In the file \texttt{equality.qrhl}\footnote{Bundled with the tool\nonanonymous{, and
...
\paragraph{The ``instructive'' proof.} This proof is formalized in \texttt{teleport.qrhl}.\footnote{Bundled with the tool\nonanonymous{, and also directly available at \giturl{teleport.qrhl}}.}
...
\texttt{Teleport.thy},\footnote{Bundled with the tool\nonanonymous{, and also
...
collection of facts from \texttt{QRHL.thy}. We omit the details.  Once
...
\texttt{teleport-terse.qrhl}.\footnote{Bundled with the tool\nonanonymous{, and also
...
\texttt{Teleport\_Terse.thy}\footnote{Bundled with the tool\nonanonymous{, and also
