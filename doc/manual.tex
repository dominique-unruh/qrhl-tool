...
%  \symbolindexmark\TOOLH\symbolindexmark\TOOLX\symbolindexmark\TOOLY\symbolindexmark\TOOLZ
  ...
  \texttt{T} any further.)
  \TODOQ{We have a command for that}
  \\
  \hline
  \subhead{States}
  %
...
\TODOQ{ensure documented: 
binary-measurement proj-classical-set  }
...
\TODOQ{Document: semiclassical tactic (using text below)}

To invoke \autoref{thm:semiclassical.search}, we use the tactic
\texttt{semiclassical}. This tactic requires that games of the exact
...
\end{figure}
%
(Again, the names of the games, as well as the variables (IN,
...
program \textrm{queryGM} is an oracle that measures first checks
...
the queried performed by the adversary $B$ in
\autoref{thm:semiclassical.search}. (Where \texttt{stop\_at} is $i$ in
$B$.)  And finally, the game \texttt{left} is like the \texttt{find}
game in tactic \texttt{o2h} (i.e., lhs of the conclusion of
\autoref{thm:semiclassical.search}) while the right game is rhs of the
conclusion of \autoref{thm:semiclassical.search}.
