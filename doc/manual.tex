...
It is also possible to include individual Isabelle commands directly inside the \texttt{.qrhl} file using the ``\texttt{isabelle\_cmd}''\toolidx{isabelle\_cmd} command.
For example:
\begin{center}
  \begin{lstlisting}
  ...
  \end{lstlisting}
\end{center}
Note that \texttt{isabelle\_cmd} must not be used inside a proof (but see the \hyperref[tactic:isa]{\texttt{isa}} tactic for using Isabelle methods).
...
When a single \texttt{+} is encountered, all current subgoals must be focused upon with \texttt{+} commands one-by-one.
...
expressions of the type \texttt{mem2 clinear\_space} (abbreviated \texttt{predicate}),
with free classical
...
of memories, and thus \texttt{mem2 clinear\_space} is the type of closed subspaces of
$\elltwov{V_1V_2}$
...
\begin{longtable}{|p{.19\hsize}|>{\parskip=\medskipamount}p{.75\hsize}|}
  ...
  \\
  \hline
  $\alpha$ \texttt{clinear\_space}\tooltype{clinear\_space} &
  ...
  \hline \texttt{predicate}\tooltype{predicate} & An abbreviation for \texttt{mem2
    clinear\_space}, that is, subspaces of
  ...
  \\
  \hline $(\alpha,\beta)$ \texttt{cblinfun}\tooltype{cblinfun} &
  ...
  \mathtt{cblinfun})$. (I.e., elements of $\boundedv{\alpha,\beta}$.)
...
\begin{longtable}{|>{\raggedright}p{.33\hsize}|>{\parskip=\medskipamount}p{.61\hsize}|}
  ...
  \texinput{\\cdot}
  \\
  \hline \constdef{$A\cdot \psi$\par$\mathtt{cblinfun\_apply}\
    A\ \psi$} {\beta}
  {$A::(\alpha,\beta)\ \mathtt{cblinfun}$
    \par and
    $\psi::\alpha$}
  ...
  {\beta\ \mathtt{clinear\_space}}
  {$A::(\alpha,\beta)\ \mathtt{cblinfun}$
    \par and
    $S::\alpha\ \mathtt{clinear\_space}$}
...
%
  ...
  {(\alpha,\beta)\ \mathtt{clinear\_space}}
  {$S::\alpha\ \mathtt{clinear\_space}$ and $ T::\beta\ \mathtt{clinear\_space}$}
  \toolconst{tensor}
...
variables\TODOQ{better example becase \texttt{frame} implements Frame}). Rules that are not yet implemented in the tool are:
...
\rulerefx{JointMeasureSimple}. \TODOQ{check whether some need to be removed from this list}
