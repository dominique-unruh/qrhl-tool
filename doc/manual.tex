...
\TODOQ{Document commands: include, cheat}

\TODOQ{Document: isabelle command can be repeated if same theory is included,
  isabelle command can have more than one theory as argument}

\TODOQ{Document: tactic squash}

\TODOQ{Document: equal-tactic arguments exclude, qvars}

\TODOQ{Document: byqrhl tactic allows 1 on lhs/rhs}

\TODOQ{Document: qrhl-command allows to give a name}

\TODOQ{Document: o2h tactic}

\TODOQ{Document: rule where x:=y; ...}

\TODOQ{Document: swap-tactic left/right x-y z., swap left/right x y. Syntax swap left discontinued}

\TODOQ{Document: swap-tactic: allows to swap overlapping read variables}
...
\end{multline*}
Here $\yy^{(1)},\dots,\yy^{(n)}$
are the free classical variables of $P,P'$.\TODOQ{Should also include fv's of $e,e'$?}
And $\qq^{(1)},\dots,\qq^{(m)}$
