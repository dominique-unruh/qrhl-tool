...
\TODOQ{Document: equal-tactic arguments:
equal [amount:nat/all] [exclude progs-comma-sep] [qvars vars-comma-sep] [innerqvars vars-comma-sep]}
...
\TODOQ{Let $X^\mathit{write}$ consist of all $x^{(i)}$
  that occur on the lhs of an assignment, sampling, or measurement. (Then $S$ is $
  \pb\paren{\{\xx^{(1)},\dots,\xx^{(n)}\}\setminus X^\mathit{write}}$-readonly.)
Let $X^\mathit{write}_j:=\idx j X^\mathit{write}$.}

First, we rewrite the postcondition $B$
in several steps. This is in order to get a new postcondition $C$ that does
not share any free \TODOQ{non-readonly} variables with $S$ (if possible) while strengthening the postcondition as little as
possible.

$B'$
is $B$
with occurrences of $\xx^{(i)}_1=\xx^{(i)}_2$
for all $i$ \TODOQ{$\xx_1=\xx_2$ for $\xx\in X^\mathit{write}$},
and occurrences of
$\llbracket \qq_1^{(1)},\dots,\qq_1^{(m)} \rrbracket
\mathrel{\mathord\equiv\mathfrak q} \llbracket
...
and $\xx^{(i)}_2$ are free variables in $B'$ \TODOQ{and $\xx^{(i)}\in X^\mathit{write}$}.
Let
\[
...
(for $j=1,2$)
such that $\xx^{(i)}$ \TODOQ{$\in X^\mathit{write}$}
