...
performing qRHL-based security proofs.  
...
Adversaries that call oracles introduce an additional subtlety: An
oracle $O$ may access a global variable $\xx$, but the adversary may
invoke $O$ in a context where $\xx$ is declared as a local
variable. This will hide the global state of $\xx$ from $O$. Thus we
need to know for which variables this happens. We say $\xx$ is an
inner variable of $A$ iff $\xx$ is not a free variable, but $\xx$ is
declared as a local variable over an oracle call. Inner variables \texttt{i1,...,in} are
declared via:
\begin{center}
  \toolidx{adversary}
  \begin{lstlisting}
    adversary name vars ... inner i1,...,in calls ...
  \end{lstlisting}
\end{center}
If \texttt{inner i1,...,in} is not given, the adversary has no inner
variables.
...
\TODOQ{Needs updating. 
  In particular local-vars treatment. 
  Options qvars, innerqvars}

This tactic is invoked as:
\[
\texttt{equal \textnormal[$n$\textnormal] \textnormal[exclude $\mathit{programs}$\textnormal]
  \textnormal[qvars $\mathit{qvars}$\textnormal]
  \textnormal[innerqvars $\mathit{qvars'}$\textnormal]}
\]
It 
converts a subgoal of the form
$\rhl{A}{\bc;\bd}{\bc';\bd'}{B}$
where $\bd,\bd'$ are the last $n$ statements (default: $n=1$) of the
left and right program
...
as well as subgoals corresponding to the differences between $\bd,\bd'$ (if any) are produced.
(I.e., the tactic assumed that $\bd,\bd'$ are similar programs, with potentially a few
statements being different.)

\bigskip

\textbf{In detail:}
By comparing $\bd,\bd'$, a context $S$ with multiple holes is obtained
such that $\bd=S[s_1,\dots,s_n]$ and $\bd'=S[s'_1,\dots,s'_n]$.\footnote{%
...
By default, the tactic attempts to find a minimal set of differences.
(In particular, if $\bd=\bd'$, then simply $S=\bd=\bd'$.)
But if the option ``\texttt{exclude} $\mathit{programs}$'' is given
(where $\mathit{programs}$ is a comma-separated list of program
names), then additionally those programs are included in the differences. 
(I.e., $S$ is guaranteed not to contain any of $\mathit{programs}$.)
...
via \texttt{call} statements, but not the programs $s_i,s_i'$ that are replaced by holes.
Let $X^\mathit{write}$ consist of all $\xx$
that occur on the lhs of an assignment, sampling, or measurement in $S$ (but not under
a \texttt{local}-statement). (Then $S$ is $
\pb\paren{X\setminus X^\mathit{write}}$-readonly.)
We write $X_1,X_2$ for the variables $X$ with added index $1,2$, and analogously
for $Q$ and $X^\mathit{write}$.


First, we rewrite the postcondition $B$ in several steps. This is in
order to get a new postcondition $C$ that does not share any written
or quantum variables with $S$ (if possible) while strengthening the
postcondition as little as possible.


$B'$
is $B$
with occurrences of $\xx_1=\xx_2$
for all $\xx\in X^\mathit{write}$,
and occurrences of
$\llbracket Q_1 \rrbracket
\mathrel{\mathord\equiv\mathfrak q} \llbracket
Q_2 \rrbracket$ removed. (In such a way that
$B'\sqcap\mathfrak{Cla}[
\forall \xx\in X^\mathit{write}.\ \xx_1=\xx_2
]\sqcap\llbracket Q_1
\rrbracket \mathrel{\mathord\equiv\mathfrak q} \llbracket
Q_2 \rrbracket$ is a subspace of $B$.)

Let $\tilde\xx^{(1)},\dots,\tilde\xx^{(k)}$ be all $\xx\in X^\mathit{write}$
that are free in $B'$.
Let
\[
 B'':=\mathfrak{Cla}\pb\parenthesis[]{\lnot (\tilde\xx_1^{(1)}=\tilde\xx_2^{(1)}\land\dots\land\xx_1^{(k)}=\xx_2^{(k)})} + B'.
\]
Note that also
$B''\sqcap\mathfrak{Cla}[
\forall \xx\in X^\mathit{write}.\ \xx_1=\xx_2
...
Let 
\[
  B''':=\mathtt{INF}\ \Hat X.\ B''
\]
where $\Hat X$
are all classical variables $\xx^{(i)}_j$
(for $j=1,2$)
such that $\xx^{(i)}\in X^\mathit{write}$, and $\xx^{(i)}_j$ is free in $B''$.

Let
\[
  C:=B'''\sqcap\mathfrak{Cla}[\xx^{(1)}_1=\xx^{(1)}_2\land\dots\land
\xx^{(n)}_1=\xx^{(k)}_2]\sqcap\llbracket Q_1
\rrbracket \mathrel{\mathord\equiv\mathfrak q} \llbracket
Q_2 \rrbracket
\qquad
\text{for } \xx^{(1)},\dots,\xx^{(k)} := X
\]
...
\TODOQ{From ...,
  we have that $S$ is $\paren{\cl{\fv(S)}\cap \Tilde X}$-readonly
  where $\Tilde X:=\cl{\fv(B''')}$ with indices removed.}\TODOQ{check}

From \ruleref{Adversary} \TODOQ{use from local-var paper}
and $\rhl{C}{s_i}{s_i'}{C}$, we have $\rhl{C}{\bd}{\bd'}{C}$
...
$\rhl{A}{\bc;\bd}{\bc';\bd'}C$. And with \ruleref{Conseq} and $C\subseteq B$,
we get $\rhl{A}{\bc;\bd}{\bc';\bd'}B$.
...
from $B$
as we can \TODOQ{(except for read-only classical ones)}\TODOQ{Continue fixup explanation}. Namely, since $C=B'''\sqcap \mathit{Eqs}$,
...
When invoked as \texttt{local left: $X$} (for some variables $X$) on a
qRHL subgoal of the form $\rhl A{\texttt{\{~local $Y$;
    $P_1$~\}}}{P_2} B$, it replaces the right program by
$\rhl A{\texttt{\{~local $X$; $P_1$~\}}}{P_2} B$.  The requirements is
$(X\symdiff Y)\cap\fv(P_1)\cap\fv(A,B) = \varnothing$. (Basically, one
cannot change local variables that occur in the pre-/postcondition.)
...
When invoked as \texttt{local left} on a
qRHL subgoal of the form $\rhl A{\texttt{\{~local $Y$;
    $P_1$~\}}}{P_2} B$, it replaces the right program by
$\rhl A{P_1}{P_2} B$.  The requirements is
$Y\cap\fv(P_1)\cap\fv(A,B) = \varnothing$. (Basically, one
cannot change local variables that occur in the pre-/postcondition.)
For classical variables, this requirement is checked by the tactic,
and for quantum variables, a new subgoal is generated for this
requirement (which can almost always be solved with \texttt{simp!}).
...
\showafter{2020}{03}{01}{\TODOQ{Document: local joint tactic (when writing local-variable paper)}}
...
\TODOQ{Document: o2h tactic}

\TODOQ{Document: semiclassical tactic}
