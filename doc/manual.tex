...
readonly variables.\footnote{That is, $F\setminus R$ is an upper bound
  for the written variables of $A$ are at most.} $O$ is a lower bound
...
$(\fv P\setminus\overwr P) \union
(\fv{P'}\setminus\overwr{P'})$.
($\fv{P}$ are the free variables, and $\overwr P$ the overwritten
...
$\qq^{(1)},\dots,\qq^{(m)}$ will simply be
$(\fv P\setminus\overwr P) \union (\fv{P'}\setminus\overwr{P'})$.
...
\TODOQ{New tactic "conseq qrhl".}

\tactic{equal}

\TODOQ{Complete rewrite needed}
