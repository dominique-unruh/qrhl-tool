...
\TODOQ{Document measure tactic}
 
\TODOQ{Document equal tactic with differences}
...
as well as $n$ ``holes'' (one for each question mark).\footnote{Formally,
  it declares \texttt{name} to refer to a context in the sense of
  \qrhlautoref{footnote:context} with $n$ holes.}
...
\begin{longtable}{|p{.275\hsize}|>{\parskip=\medskipamount}p{.66\hsize}|}
  ...
  where each \texttt{arg1,\dots,argn} is again the name of a program or such an expression.
