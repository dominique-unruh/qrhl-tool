...
At this point, it is not yet meant for larger developments.
...
\item \texttt{isabelle-home = \textnormal{\textit{dir}}} \\
  ...
  (I.e., the directory that contains files such as \texttt{Isabelle2022}, \texttt{ROOTS}, etc.)
...
interpreted by Isabelle/HOL (currently the Isabelle/HOL 2022 version), in the
...
It can be applied to goals comparing probabilities (e.g., \texttt{Pr[...] <= Pr[...]}) as well as to denotational-equivalence goals.
...
&
  \qquad\qquad
  \begin{aligned}
    &\text{with} && e_1 := \idx1e,\ e'_2 := \idx2e'
\\
&\text{and} &&  A := \CL{\yy^{(1)}_1=\yy^{(1)}_2\land\dots\land\yy^{(n)}_1=\yy^{(n)}_2}
\\
&&&\qquad\quad{}\sqcap
    \llbracket \qq_1^{(1)},\dots,\qq_1^{(m)} \rrbracket
    \mathrel{\mathord\equiv\mathfrak q}
...
This tactic allows to replace a sequence of lines in the left or right program by different code.
A new subgoal stating the denotational equivalence between those lines and the replacement code is produced.
...
This tactic will replace lines $n$--$m$ in that program by the code specified by \textit{replacement}.
...
\paragraph{Example file.} An example of this tactic in use is given in the example file \texttt{examples/rewrite.qrhl}.
...
Compared with the \tacticref{wp} tactic, there are four important differences:
\begin{itemize}
...
For example, the block $\bc$ (specified by $\mathit{range_1}$) could have a single line $\be_1$ of code that does have a shared quantum variable with the block $\bd$ (specified by $\mathit{range_2}$).
...
\end{lstlisting}
Here $\be_1$ to $\be_m$ are program fragments (in the syntax from \autoref{sec:tool.programs}) of the block $\bc$ (selected by $\mathit{range_1}$) that should be excluded from the variable disjointness conditions.\footnote{%
...
Note: The \tacticref{byqrhl} tactic can be used to prove the resulting denotational-equivalence subgoals.
...
\end{lstlisting}
This will swap the two statements, and exclude the program call to \texttt{P} in \texttt{call A(P)} from the requirement to not contain the variable \texttt{x}.
...
The subprograms $\be_i$ are only searched for in $\mathit{range_1}$ (i.e., in block $\bc$), not in $\mathit{range_2}$.
...
\item The inner variables\footnote{That is, local variables of $C$ that have a hole of $C$ in their scope.} of $C$ must be disjoint from the free variables of $\bd$.
\end{itemize}
...
\item If $\mathit{range_1}$ comes before $\mathit{range_2}$ in the program:
  ...
  \end{itemize}
\item If $\mathit{range_2}$ comes before $\mathit{range_1}$ in the program:
  \begin{itemize}
  \item For each $i=1,\dots,n$: Denotational equivalence of   $\bd;\be_i$ and ${\be_i;\bd}$.
  \item The original subgoal with $\bd;\bc$ replaced by $\bc;\bd$.
  \end{itemize}
\end{itemize}
...
We cannot just swap \texttt{x <- x + 1} and \texttt{call A(P)} because \texttt{x <- x + 1} and \texttt{call P} do not commute.
However, we can swap \texttt{x <- x + 1} and \texttt{call A(P)} if we replace \texttt{P} by \texttt{P' := \{ y <- 2 * (x + 1) \}} after swapping.
That is, we can rewrite program $(*)$ into \texttt{call A(P'); x <- x + 1;}, and all we need to prove is that \texttt{x <- x + 1; call P;} and \texttt{call P'; x <- x + 1;} are denotationally equivalent (which is easy given the definitions of \texttt{P} and \texttt{P'}).

This can be achieved by using an even more general form of the \texttt{swap} tactic.
...
\item If $\mathit{range_2}$ comes before $\mathit{range_1}$ in the program:
  \begin{itemize}
  \item For each $i=1,\dots,n$: Denotational equivalence of $\bd;\be_i$ and ${\be_i';\bd}$.
  \item The original subgoal with $\bd;\bc$ replaced by $C[\be_1',\dots,\be_m];\bd$.
  \end{itemize}
\end{itemize}
...
sorts can be consistently assumed.  If this is not the case, an error
is issued.\footnote{If the warning is not justified, it is possible to
  ...
  \texttt{declare\_variable\_type} will not issue an error any more
  since the existence of a type of the right sort is ensured.
  Boilerplate code for these manual steps are provided in the error message.}
For example,
\begin{lstlisting}
  declare_variable_type wrong :: "{finite,no_top}"
\end{lstlisting}
produces an error since it would declare a type that is both finite
