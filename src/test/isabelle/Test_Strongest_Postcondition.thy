...
test_get_sp \<^context> true
            ...
            \<^term>\<open>Expr[Cla[a1=0]]\<close> (* pre *)
            \<^term>\<open>Expr[SUP z::bit. \<CC>\<ll>\<aa>[a1 \<in> supp (uniform UNIV)] \<sqinter> \<CC>\<ll>\<aa>[z = (0::bit)]] :: _ expression2\<close> (* expected *)
\<close>
...
test_get_sp \<^context> true
            ...
            \<^term>\<open>Expr[(SUP z::bit. \<CC>\<ll>\<aa>[a1 = (1::bit)] \<sqinter> (\<CC>\<ll>\<aa>[z = (0::bit)] \<sqinter> \<top>)) \<squnion>
                         (SUP z::bit. \<CC>\<ll>\<aa>[a1 = (0::bit)] \<sqinter> (\<CC>\<ll>\<aa>[z \<noteq> (0::bit)] \<sqinter> \<top>))]\<close> (* expected *)
\<close>
...
test_get_sp \<^context> true
            ...
            \<^term>\<open>Expr[\<lbrakk>A1\<rbrakk> =\<^sub>q ket 0] :: _ expression2\<close> (* pre *)
            \<^term>\<open>Expr[hadamard\<guillemotright>\<lbrakk>A1\<rbrakk> *\<^sub>S \<lbrakk>A1\<rbrakk> =\<^sub>\<qq> |0::bit\<rangle>] :: _ expression2\<close> (* expected *)
\<close>
