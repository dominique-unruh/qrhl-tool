...
import qrhl.logic.{Assign, ExpressionIndexed, QVariable}
...
class EqualTacTest extends AnyFunSuite {
  ...
  test("SimpleQeq") {
    ...
    val e = ExpressionIndexed.fromString(tl.state.isabelle.context, "Qeq[q1,r1 = q2,r2]", globalIsabelle.predicateT)
    println(e)
    val t = e.instantiateMemory.termInst.isabelleTerm
    println(IsabelleX.pretty(t))
    t match {
      case `simpleQeq`(vars) =>
        println(vars)
