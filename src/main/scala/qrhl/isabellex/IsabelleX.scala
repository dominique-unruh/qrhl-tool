...
/*  def variable_singleton(typ: Typ): Const = Const(c.variable_singleton, variableT(typ) -->: variablesT(typ))
  ...
  object Ops {
    ...
    val decodeFromExpressionOp =
      MLValue.compileFunction[Context,Term, Term](s"fn (ctxt, t) => $qrhl_ops.expression_to_term ctxt t")
  ...
  def checkAFPVersion(): Unit = Configuration.afpRoot match {
    ...
    case Some(path) =>
      ...
      val hashes = Seq(
        ...
        "thys/Hilbert_Space_Tensor_Product/Hilbert_Space_Tensor_Product.thy" -> "a9aeeb5aade3c6d0302340c21540db55a9b20cd83fcee7cb70457f5ce3d5b8ac",
      )
