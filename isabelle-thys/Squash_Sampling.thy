...
proof -
  ...
  also have \<open>\<dots> = denotation (sample (variable_concat Q R) de)\<close>
...
proof -
  have \<open>denotation (block [assign Q d, assign R e])
    ...
    by (auto simp: denotation_block assign_sample)
  also have \<open>\<dots> = denotation (assign (variable_concat Q R) de)\<close>
...
schematic_goal xxx: "qrhl A
 [sample \<lbrakk>z\<rbrakk> Expr[uniform UNIV], sample \<lbrakk>x\<rbrakk> Expr[uniform {1,z}], sample \<lbrakk>y\<rbrakk> Expr[uniform {x,z}]]
