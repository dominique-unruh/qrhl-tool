...
(* definition \<open>cq_map_rel \<EE> \<FF> \<longleftrightarrow> (\<forall>x. kf_norm (\<EE> x) \<le> 1 \<and> kraus_equivalent' (\<EE> x) (\<FF> x))\<close>
...
(* lemma cq_map_rel_refl:
  ...
  assumes \<open>\<And>x. kf_norm (\<EE> x) \<le> 1\<close>
...
typedef ('cl1,'qu1,'cl2,'qu2) cq_map = \<open>{E :: 'cl1 \<Rightarrow> ('qu1 ell2, 'qu2 ell2, 'cl2) kraus_family. bdd_above (range (\<lambda>x. kf_norm (E x)))}\<close>
  by (rule exI[of _ 0], simp)
setup_lifting type_definition_cq_map

lift_definition cq_map_id :: \<open>('cl,'qu,'cl,'qu) cq_map\<close> is 
  \<open>\<lambda>c. kf_map_outcome (\<lambda>(). c) kf_id\<close>
  by simp
...
lift_definition cq_map_apply :: \<open>('cl1,'qu1,'cl2,'qu2) cq_map \<Rightarrow> ('cl1,'qu1) cq_operator \<Rightarrow> ('cl2,'qu2) cq_operator\<close> is
  \<open>\<lambda>\<EE> \<rho> c. (\<Sum>\<^sub>\<infinity>d. kf_apply' {c} (\<EE> d) (\<rho> d))\<close>
proof (rename_tac \<EE> \<rho>, intro conjI allI ext)
  ...
  assume \<open>bdd_above (range (\<lambda>x. kf_norm (\<EE> x)))\<close>
  then obtain N where norm_\<EE>: \<open>kf_norm (\<EE> x) \<le> N\<close> for x
  ...
  from \<rho>_pos
  show \<open>0 \<le> (\<Sum>\<^sub>\<infinity>d. kf_apply' {c} (\<EE> d) (\<rho> d))\<close>
    by (auto intro!: infsum_nonneg_traceclass kf_apply'_pos)

  from \<rho>_pos
  have 9: \<open>trace_tc (kf_apply (\<EE> d) (\<rho> d)) \<le> kf_norm (\<EE> d) * trace_tc (\<rho> d)\<close> for d
    by (smt (verit) Extra_Ordered_Fields.complex_of_real_cmod cmod_mono complex_of_real_nn_iff 
        kf_apply_bounded_pos abs_pos kf_apply_pos nn_comparable norm_tc_pos of_real_hom.hom_mult 
  ...
  have 0: \<open>0 \<le> Re (trace_tc (kf_apply' {c} (\<EE> d) (\<rho> d)))\<close> for c d
    by (metis Re_complex_of_real \<rho>_pos kf_apply'_pos norm_cblinfun_mono_trace_class norm_tc_pos norm_zero order_eq_refl)
  have 3: \<open>(\<Sum>\<^sub>\<infinity>c. kf_apply' {c} (\<EE> d) (\<rho> d)) = kf_apply (\<EE> d) (\<rho> d)\<close> for d
  proof -
    have \<open>(\<Sum>\<^sub>\<infinity>c. kf_apply' {c} (\<EE> d) (\<rho> d)) = (\<Sum>\<^sub>\<infinity>X\<in>range (\<lambda>c. {c}). kf_apply' X (\<EE> d) (\<rho> d))\<close>
      by (simp add: infsum_reindex o_def)
    also have \<open>\<dots> = kf_apply' (\<Union>(range (\<lambda>c. {c}))) (\<EE> d) (\<rho> d)\<close>
      apply (rule kf_apply'_union_infsum)
      by auto
    also have \<open>\<dots> = kf_apply (\<EE> d) (\<rho> d)\<close>
      by simp
    finally show ?thesis
      by -
  qed
  have 4: \<open>(\<lambda>c. kf_apply' {c} (\<EE> d) (\<rho> d)) summable_on UNIV\<close> for d
  proof -
    have \<open>(\<lambda>X. kf_apply' X (\<EE> d) (\<rho> d)) summable_on range (\<lambda>c. {c})\<close> for d
      apply (rule kf_apply'_union_summable_on)
      by auto
    then show ?thesis
  ...
  from 3 4 have 5: \<open>(\<Sum>\<^sub>\<infinity>c. Re (trace_tc (kf_apply' {c} (\<EE> d) (\<rho> d)))) = Re (trace_tc (kf_apply (\<EE> d) (\<rho> d)))\<close> for d
    apply (subst infsum_bounded_linear[where h=\<open>Re o trace_tc\<close>, unfolded o_def])
    using 16 by auto
  have 13: \<open>0 \<le> Re (trace_tc (kf_apply (\<EE> d) (\<rho> d)))\<close> for d
    by (metis Re_complex_of_real \<rho>_pos kf_apply_pos norm_cblinfun_mono_trace_class norm_tc_pos norm_zero order_eq_refl)
  ...
  from 9 10 have 14: \<open>Re (trace_tc (kf_apply (\<EE> d) (\<rho> d))) \<le> N * Re (trace_tc (\<rho> d))\<close> for d
    using dual_order.trans Re_mono
    by (metis scaleR_complex.simps(1) scaleR_conv_of_real)
  have 6: \<open>(\<lambda>d. Re (trace_tc (kf_apply (\<EE> d) (\<rho> d)))) summable_on UNIV\<close>
  ...
  have 29: \<open>(\<lambda>c. Re (trace_tc (kf_apply' {c} (\<EE> d) (\<rho> d)))) summable_on UNIV\<close> for d
    apply (rule summable_on_bounded_linear[where h=\<open>\<lambda>x. Re (trace_tc x)\<close>])
    using 16 4 by auto
  have 18: \<open>(\<lambda>(d,c). Re (trace_tc (kf_apply' {c} (\<EE> d) (\<rho> d)))) summable_on (UNIV \<times> UNIV)\<close>
    apply (rule summable_on_SigmaI[where g=\<open>\<lambda>d. \<Sum>\<^sub>\<infinity>c. Re (trace_tc (kf_apply' {c} (\<EE> d) (\<rho> d)))\<close>])
    using 0 5 6 29 by (auto intro!: has_sumI)
  have 21: \<open>(\<lambda>d. Re (trace_tc (kf_apply' {c} (\<EE> d) (\<rho> d)))) summable_on UNIV\<close> for c
    using summable_on_SigmaD1[OF summable_on_swap[THEN iffD1, OF 18]]
    by auto
  have 22: \<open>(\<lambda>c. \<Sum>\<^sub>\<infinity>d. Re (trace_tc (kf_apply' {c} (\<EE> d) (\<rho> d)))) summable_on UNIV\<close>
    apply (rule summable_on_Sigma_banach)
    apply (rule summable_on_swap[THEN iffD2])
    using 18 by simp
  have 36: \<open>norm (kf_apply' {c} (\<EE> d) (\<rho> d)) = Re (trace_tc (kf_apply' {c} (\<EE> d) (\<rho> d)))\<close> for c d
    apply (rule norm_tc_pos_Re)
    by (simp add: \<rho>_pos kf_apply'_pos)
  from 36 have 35: \<open>(\<lambda>d. kf_apply' {c} (\<EE> d) (\<rho> d)) abs_summable_on UNIV\<close> for c
    by (simp add: 36 21)
  from 35 have 34: \<open>(\<lambda>d. kf_apply' {c} (\<EE> d) (\<rho> d)) summable_on UNIV\<close> for c
    by (rule abs_summable_summable)
  from 22 have 23: \<open>(\<lambda>c. Re (trace_tc (\<Sum>\<^sub>\<infinity>d. kf_apply' {c} (\<EE> d) (\<rho> d)))) summable_on UNIV\<close>
    apply (rule summable_on_cong[THEN iffD1, rotated])
    using 16 34 by (rule infsum_bounded_linear[unfolded o_def])
  from 23 \<rho>_pos show \<open>(\<lambda>c. \<Sum>\<^sub>\<infinity>d. kf_apply' {c} (\<EE> d) (\<rho> d)) abs_summable_on UNIV\<close>
    by (simp add: infsum_nonneg_traceclass kf_apply'_pos norm_tc_pos_Re)
qed
...
proof (rename_tac \<EE> \<EE>' \<rho>, intro conjI allI ext)
  ...
  then have norm_\<EE>: \<open>kf_norm (\<EE> x) \<le> 1\<close> for x
  ...
  from \<rho>_pos have 10: \<open>\<dots>d \<le> trace_tc (\<rho> d)\<close> for d
    by (smt (verit, del_insts) Rings.mult_sign_intros(1) cmod_Re complex_of_real_leq_1_iff complex_of_real_nn_iff kf_norm_geq0 less_eq_complex_def mult_left_le_one_le norm_\<EE> norm_ge_zero norm_mult norm_one trace_tc_pos)
  have 0: \<open>0 \<le> Re (trace_tc (kf_apply' {c} (\<EE> d) (\<rho> d)))\<close> for c d
    by (metis Re_complex_of_real \<rho>_pos kf_apply'_pos norm_cblinfun_mono_trace_class norm_tc_pos norm_zero order_eq_refl)
  have 3: \<open>(\<Sum>\<^sub>\<infinity>c. kf_apply' {c} (\<EE> d) (\<rho> d)) = kf_apply (\<EE> d) (\<rho> d)\<close> for d
  proof -
  ...
  from 9 10 have 14: \<open>Re (trace_tc (kf_apply (\<EE> d) (\<rho> d))) \<le> Re (trace_tc (\<rho> d))\<close> for d
  ...
  from 6 have 11: \<open>(\<Sum>\<^sub>\<infinity>d. Re (trace_tc (kf_apply (\<EE> d) (\<rho> d)))) \<le> (\<Sum>\<^sub>\<infinity>d. Re (trace_tc (\<rho> d)))\<close>
  ...
  from 5 6 have 2: \<open>(\<lambda>d. \<Sum>\<^sub>\<infinity>c. Re (trace_tc (kf_apply' {c} (\<EE> d) (\<rho> d)))) summable_on UNIV\<close>
    by simp
  have 17: \<open>(\<Sum>\<^sub>\<infinity>d. \<Sum>\<^sub>\<infinity>c. Re (trace_tc (kf_apply' {c} (\<EE> d) (\<rho> d)))) \<le> 1\<close>
    using 5 15 11
    by auto
  have 29: \<open>(\<lambda>c. Re (trace_tc (kf_apply' {c} (\<EE> d) (\<rho> d)))) summable_on UNIV\<close> for d
    apply (rule summable_on_bounded_linear[where h=\<open>\<lambda>x. Re (trace_tc x)\<close>])
  ...
  have 22: \<open>(\<lambda>c. \<Sum>\<^sub>\<infinity>d. Re (trace_tc (kf_apply' {c} (\<EE> d) (\<rho> d)))) summable_on UNIV\<close>
    ...
    apply (rule summable_on_swap[THEN iffD2])
    using 18 by simp
  have 20: \<open>(\<lambda>(c, d). Re (trace_tc (kf_apply' {c} (\<EE> d) (\<rho> d)))) summable_on UNIV\<close>
    apply (subst asm_rl[of \<open>UNIV = UNIV \<times> UNIV\<close>], simp)
    apply (rule summable_on_SigmaI[where g=\<open>\<lambda>c. \<Sum>\<^sub>\<infinity>d. Re (trace_tc (kf_apply' {c} (\<EE> d) (\<rho> d)))\<close>])
    using 21 22 0 by (auto intro!: has_sum_infsum)
  from 17 18 20 have 19: \<open>(\<Sum>\<^sub>\<infinity>c. \<Sum>\<^sub>\<infinity>d. Re (trace_tc (kf_apply' {c} (\<EE> d) (\<rho> d)))) \<le> 1\<close>
  ...
  from 22 have 23: \<open>(\<lambda>c. Re (trace_tc (\<Sum>\<^sub>\<infinity>d. kf_apply' {c} (\<EE> d) (\<rho> d)))) summable_on UNIV\<close>
    apply (rule summable_on_cong[THEN iffD1, rotated])
    using 16 34 by (rule infsum_bounded_linear[unfolded o_def])
  have 25: \<open>(\<Sum>\<^sub>\<infinity>c. \<Sum>\<^sub>\<infinity>d. trace_tc (kf_apply' {c} (\<EE> d) (\<rho> d))) \<ge> 0\<close>
    by (auto intro!: infsum_nonneg_complex trace_tc_pos kf_apply'_pos \<rho>_pos)
  from 23 show \<open>(\<lambda>c. trace_tc (\<Sum>\<^sub>\<infinity>d. kf_apply' {c} (\<EE> d) (\<rho> d))) summable_on UNIV\<close>
    ...
    by (auto intro!: nonnegative_complex_is_real summable_on_bounded_linear[where h=of_real]
        bounded_linear_of_real trace_tc_pos  infsum_nonneg_traceclass kf_apply'_pos \<rho>_pos)
  have 26: \<open>(\<Sum>\<^sub>\<infinity>c. Re (trace_tc (\<Sum>\<^sub>\<infinity>d. kf_apply' {c} (\<EE> d) (\<rho> d)))) \<le> 1\<close>
    apply (subst infsum_bounded_linear[OF 16, symmetric, unfolded o_def])
    using 19 34 by auto
  from 26 show \<open>(\<Sum>\<^sub>\<infinity>c. trace_tc (\<Sum>\<^sub>\<infinity>d. kf_apply' {c} (\<EE> d) (\<rho> d))) \<le> 1\<close>
    apply (rewrite at \<open>trace_tc _\<close> of_real_Re[symmetric])
    by (auto intro!: nonnegative_complex_is_real summable_on_bounded_linear[where h=of_real]
        bounded_linear_of_real trace_tc_pos  infsum_nonneg_traceclass kf_apply'_pos \<rho>_pos
        simp: infsum_of_real )
  from rel
  show \<open>(\<Sum>\<^sub>\<infinity>d. kf_apply' {c} (\<EE> d) (\<rho> d)) = (\<Sum>\<^sub>\<infinity>d. kf_apply' {c} (\<EE>' d) (\<rho> d))\<close>
    by (auto intro!: kf_apply'_eqI infsum_cong simp: cq_map_rel_def)
qed
 *)
...
lift_definition cq_map_seq :: \<open>('cl1,'qu1,'cl2,'qu2) cq_map \<Rightarrow> ('cl2,'qu2,'cl3,'qu3) cq_map \<Rightarrow> ('cl1,'qu1,'cl3,'qu3) cq_map\<close>
  is \<open>\<lambda>\<EE> \<FF> c. kf_map_outcome snd (kf_comp_dependent \<FF> (\<EE> c))\<close>
proof (rename_tac \<EE> \<FF>)
  fix \<EE> :: \<open>'cl1 \<Rightarrow> ('qu1 ell2, 'qu2 ell2, 'cl2) kraus_family\<close>
  fix \<FF> :: \<open>'cl2 \<Rightarrow> ('qu2 ell2, 'qu3 ell2, 'cl3) kraus_family\<close>
  assume asms: \<open>bdd_above (range (\<lambda>x. kf_norm (\<EE> x)))\<close> \<open>bdd_above (range (\<lambda>x. kf_norm (\<FF> x)))\<close>
  then obtain F where F1: \<open>kf_norm (\<FF> x) \<le> F\<close> for x
    by (auto intro!: simp: bdd_above_def)
  from kf_norm_geq0 this have \<open>F \<ge> 0\<close>
    by (rule order_trans)
  from asms obtain E where E1: \<open>kf_norm (\<EE> x) \<le> E\<close> for x
    by (auto intro!: simp: bdd_above_def)
  have \<open>kf_norm (kf_comp_dependent \<FF> (\<EE> x)) \<le> F * kf_norm (\<EE> x)\<close> for x
    apply (rule kf_comp_dependent_norm)
    by (simp add: F1)
  also have \<open>\<dots>x \<le> F * E\<close> for x
    by (intro mult_left_mono E1 \<open>F \<ge> 0\<close>)
  finally have 1: \<open>kf_norm (kf_comp_dependent \<FF> (\<EE> x)) \<le> F * E\<close> for x
    by simp
  then show \<open>bdd_above (range (\<lambda>x. kf_norm (kf_map_outcome snd (kf_comp_dependent \<FF> (\<EE> x)))))\<close>
    by (auto intro!: bdd_aboveI simp: kf_map_outcome_norm)
qed
...
proof (intro iffI ext)
  ...
  then show \<open>cq_map_apply s \<rho> = cq_map_apply t \<rho>\<close>
    apply transfer'
    by (intro ext infsum_cong kf_apply'_eqI)
next
  ...
  then have *: \<open>Rep_cq_operator (cq_map_apply s \<rho>) = Rep_cq_operator (cq_map_apply t \<rho>)\<close> for \<rho>
    by simp
  (* then have \<open>(\<Sum>\<^sub>\<infinity>d. kf_apply' {c} (Rep_cq_map s d) (\<rho> d)) = (\<Sum>\<^sub>\<infinity>d. kf_apply' {c} (Rep_cq_map t d) (\<rho> d))\<close> for c *)
  ...
  proof (rule kraus_equivalent'I)
    ...
    then have **: \<open>(\<Sum>\<^sub>\<infinity>d. kf_apply' {c} (Rep_cq_map s d) (of_bool (x = d) *\<^sub>R \<rho>))
             = (\<Sum>\<^sub>\<infinity>d. kf_apply' {c} (Rep_cq_map t d) (of_bool (x = d) *\<^sub>R \<rho>))\<close>
      apply (simp add: cq_map_apply.rep_eq fixed_cl_cq_operator.rep_eq)
      by meson
    show \<open>kf_apply' {c} (Rep_cq_map s x) \<rho> = kf_apply' {c} (Rep_cq_map t x) \<rho>\<close>
      using **
...
proof transfer'
  ...
  have \<open>(\<Sum>\<^sub>\<infinity>d. kf_apply' {c} (kf_map_outcome (\<lambda>(). d) kf_id) (\<rho> d)) = \<rho> c\<close> for c
    apply (subst infsum_single[where i=c])
    by (auto intro!: simp: kf_apply'_map_outcome)
  then show \<open>(\<lambda>c. \<Sum>\<^sub>\<infinity>d. kf_apply' {c} (kf_map_outcome (\<lambda>(). d) kf_id) (\<rho> d)) = \<rho>\<close>
    by auto
qed
...
proof (transfer, intro ext)
  ...
  assume bdd_s: \<open>bdd_above (range (\<lambda>x. kf_norm (s x)))\<close>
  then obtain S where S: \<open>kf_norm (s x) \<le> S\<close> for x
    by (auto intro!: simp: bdd_above_def)
  assume bdd_t: \<open>bdd_above (range (\<lambda>x. kf_norm (t x)))\<close>
  then obtain T where T: \<open>kf_norm (t x) \<le> T\<close> for x
    by (auto intro!: simp: bdd_above_def)
  have [simp]: \<open>T \<ge> 0\<close>
    using kf_norm_geq0 T
    by (rule order_trans)
  have inj: \<open>inj_on fst (Set.filter (\<lambda>(E, x). x = f) X)\<close> for X f
    by (auto intro!: inj_onI)
  from bdd_t have bdd': \<open>bdd_above (range (kf_norm \<circ> (\<lambda>e. kf_filter (\<lambda>f. f = c) (t e))))\<close>
  ...
  have summ2: \<open>(\<lambda>d. kf_apply' {f} (s d) (\<rho> d)) summable_on UNIV\<close> for f
  proof (rule abs_summable_summable, rule abs_summable_on_comparison_test)
    from \<rho>_abs show \<open>(\<lambda>d. S *\<^sub>R \<rho> d) abs_summable_on UNIV\<close>
      by blast
    fix d
    have \<open>norm (kf_apply' {f} (s d) (\<rho> d)) \<le> kf_norm (kf_filter (\<lambda>x. x = f) (s d)) * norm (\<rho> d)\<close>
      using assms by (auto intro!: kf_apply_bounded_pos simp add: kf_apply'_def)
    also have \<open>\<dots> \<le> S * norm (\<rho> d)\<close>
      apply (rule mult_right_mono)
      using S kf_norm_filter[of \<open>\<lambda>x. x = f\<close> \<open>s d\<close>]
       apply (auto intro!: simp: )
      by (smt (verit, del_insts))
    also have \<open>\<dots> = norm (S *\<^sub>R \<rho> d)\<close>
      by (smt (verit, del_insts) S kf_norm_geq0 norm_scaleR)
    finally show \<open>norm (kf_apply' {f} (s d) (\<rho> d)) \<le> norm (S *\<^sub>R \<rho> d)\<close>
      by -
  qed
  ...
  proof -
    ...
    from kf_apply_summable[of _ \<open>s c\<close>]
  ...
  qed
  have summ4: \<open>(\<lambda>y. kf_apply' {c} (t y) (kf_apply' {y} (s d) (\<rho> d))) summable_on UNIV\<close> for d
  proof -
    have 1: \<open>(\<lambda>f. T *\<^sub>R kf_apply' {f} (s d) (\<rho> d)) abs_summable_on UNIV\<close>
    proof -
      have \<open>(\<lambda>X. kf_apply' X (s d) (\<rho> d)) summable_on range (\<lambda>f. {f})\<close>
        apply (rule kf_apply'_union_summable_on)
        by auto
      then have \<open>(\<lambda>f. kf_apply' {f} (s d) (\<rho> d)) summable_on UNIV\<close>
        apply (subst (asm) summable_on_reindex)
        by (simp_all add: o_def)
      then show ?thesis
        apply (intro summable_abs_summable_tc scaleR_nonneg_nonneg kf_apply'_pos)
        using assms by auto
    qed
    have 2: \<open>norm (kf_apply' {c} (t f) (kf_apply' {f} (s d) (\<rho> d)))
         \<le> norm (T *\<^sub>R kf_apply' {f} (s d) (\<rho> d))\<close> for f
    proof -
      have \<open>norm (kf_apply' {c} (t f) (kf_apply' {f} (s d) (\<rho> d)))
        \<le> kf_norm (kf_filter (\<lambda>x. x\<in>{c}) (t f)) * norm (kf_apply' {f} (s d) (\<rho> d))\<close>
        using assms
        by (auto intro!: kf_apply_bounded_pos
            simp: kf_apply'_def kf_apply_pos)
      also have \<open>\<dots> \<le> T * norm (kf_apply' {f} (s d) (\<rho> d))\<close>
        apply (rule mult_right_mono)
        using T
         apply (smt (verit, ccfv_threshold) kf_norm_filter)
        by simp
      also have \<open>\<dots> = norm (T *\<^sub>R kf_apply' {f} (s d) (\<rho> d))\<close>
        by (smt (verit) T kf_norm_geq0 norm_scaleR)
      finally show ?thesis
        by simp
    qed
    show ?thesis
  ...
  have \<open>(\<Sum>\<^sub>\<infinity>d. kf_apply' {c} (kf_map_outcome snd (kf_comp_dependent t (s d))) (\<rho> d))
     = (\<Sum>\<^sub>\<infinity>d. kf_apply (kf_filter (\<lambda>(e, f). f=c \<and> True) (kf_comp_dependent t (s d))) (\<rho> d))\<close>
    by (simp add: kf_apply'_def kf_filter_map_outcome case_prod_unfold)
  also have \<open>\<dots> = (\<Sum>\<^sub>\<infinity>d. kf_apply (kf_comp_dependent (\<lambda>e. kf_filter (\<lambda>f. f = c) (t e)) (s d)) (\<rho> d))\<close>
    apply (subst kf_filter_comp_dependent)
    using bdd_t by simp_all
  also have \<open>\<dots> = (\<Sum>\<^sub>\<infinity>d. \<Sum>\<^sub>\<infinity>(F, f)\<in>Rep_kraus_family (s d). kf_apply' {c} (t f) (sandwich_tc F (\<rho> d)))\<close>
    using bdd'
    by (simp add: kf_comp_dependent_apply kf_apply'_def)
  also have \<open>\<dots> = (\<Sum>\<^sub>\<infinity>d. \<Sum>\<^sub>\<infinity>(f, F)\<in>prod.swap ` Rep_kraus_family (s d). kf_apply' {c} (t f) (sandwich_tc F (\<rho> d)))\<close>
    apply (subst infsum_reindex)
    by (auto intro!: inj_onI simp: o_def)
  also have \<open>\<dots> = (\<Sum>\<^sub>\<infinity>d. \<Sum>\<^sub>\<infinity>(f, F)\<in>(SIGMA f:UNIV. {F. (F, f) \<in> Rep_kraus_family (s d)}). kf_apply' {c} (t f) (sandwich_tc F (\<rho> d)))\<close>
    apply (rule infsum_cong)
    apply (rule arg_cong[where f=\<open>infsum _\<close>])
    by fastforce
  also have \<open>\<dots> = (\<Sum>\<^sub>\<infinity>d. \<Sum>\<^sub>\<infinity>f. \<Sum>\<^sub>\<infinity>F\<in>{F. (F,f) \<in> Rep_kraus_family (s d)}. kf_apply' {c} (t f) (sandwich_tc F (\<rho> d)))\<close>
    apply (rule infsum_cong)
    apply (subst infsum_Sigma_positive_tc)
    using assms by (auto intro!: summ3 kf_apply'_pos sandwich_tc_pos
        summable_on_bounded_linear[where h=\<open>kf_apply' _ _\<close>]
        bounded_clinear.bounded_linear kf_apply'_bounded_clinear)
  also have \<open>\<dots> = (\<Sum>\<^sub>\<infinity>d. \<Sum>\<^sub>\<infinity>f. kf_apply' {c} (t f) (\<Sum>\<^sub>\<infinity>F\<in>{F. (F,f) \<in> Rep_kraus_family (s d)}. sandwich_tc F (\<rho> d)))\<close>
    apply (intro infsum_cong)
    apply (rule infsum_bounded_linear[unfolded o_def])
    by (auto intro!: bounded_clinear.bounded_linear kf_apply'_bounded_clinear summ3)
  also have \<open>\<dots> = (\<Sum>\<^sub>\<infinity>d. \<Sum>\<^sub>\<infinity>f. kf_apply' {c} (t f) (kf_apply' {f} (s d) (\<rho> d)))\<close>
    apply (auto intro!: infsum_cong arg_cong[where f=\<open>kf_apply' _ _\<close>]
        simp: kf_apply'_def kf_apply.rep_eq kf_filter.rep_eq)
  ...
  also have \<open>\<dots> = (\<Sum>\<^sub>\<infinity>f. \<Sum>\<^sub>\<infinity>d. kf_apply' {c} (t f) (kf_apply' {f} (s d) (\<rho> d)))\<close>
    apply (rule infsum_swap_positive_tc)
    using assms
    by (auto intro!: summ4 summ2 summable_on_bounded_linear[where h=\<open>kf_apply' _ _\<close>]
        bounded_clinear.bounded_linear kf_apply'_bounded_clinear kf_apply'_pos simp: )
  also have \<open>\<dots> = (\<Sum>\<^sub>\<infinity>f. kf_apply' {c} (t f) (\<Sum>\<^sub>\<infinity>d. kf_apply' {f} (s d) (\<rho> d)))\<close>
    apply (intro infsum_cong)
    apply (rule infsum_bounded_linear[unfolded o_def])
    by (auto intro!: bounded_clinear.bounded_linear kf_apply'_bounded_clinear summ2)
  finally  show \<open>(\<Sum>\<^sub>\<infinity>d. kf_apply' {c} (kf_map_outcome snd (kf_comp_dependent t (s d))) (\<rho> d)) =
       (\<Sum>\<^sub>\<infinity>f. kf_apply' {c} (t f) (\<Sum>\<^sub>\<infinity>d. kf_apply' {f} (s d) (\<rho> d)))\<close>
    by -
qed
...
proof (rename_tac e \<EE> \<FF>)
  ...
  assume \<open>bdd_above (range (\<lambda>x. kf_norm (\<EE> x)))\<close>
  then obtain B where B: \<open>kf_norm (\<EE> x) \<le> B\<close> for x
    by (auto intro!: simp: bdd_above_def)
  assume \<open>bdd_above (range (\<lambda>x. kf_norm (\<FF> x)))\<close>
  then obtain C where C: \<open>kf_norm (\<FF> x) \<le> C\<close> for x
    by (auto intro!: simp: bdd_above_def)
  from B C
  have \<open>kf_norm (if e x then \<EE> x else \<FF> x) \<le> max B C\<close> for x
    by smt
  then show \<open>bdd_above (range (\<lambda>x. kf_norm (if e x then \<EE> x else \<FF> x)))\<close>
...
lift_definition cq_map_quantum_op :: \<open>('cl \<Rightarrow> ('qu1 ell2, 'qu2 ell2, 'x) kraus_family) \<Rightarrow> ('cl,'qu1,'cl,'qu2) cq_map\<close> is
  \<open>\<lambda>(\<EE>::'cl \<Rightarrow> ('qu1 ell2, 'qu2 ell2, 'x) kraus_family) c. 
      if bdd_above (range (\<lambda>c. kf_norm (\<EE> c))) then kf_map_outcome (\<lambda>_. c) (\<EE> c) else 0\<close>
...
definition cq_map_of_op :: \<open>'qu1 ell2 \<Rightarrow>\<^sub>C\<^sub>L 'qu2 ell2 \<Rightarrow> ('cl, 'qu1, 'cl, 'qu2) cq_map\<close> where
  \<open>cq_map_of_op U = cq_map_quantum_op (\<lambda>_. kf_of_op U)\<close>
...
lift_definition cq_map_tensor_op_right :: \<open>('extra ell2, 'extra ell2) trace_class \<Rightarrow> ('cl, 'qu, 'cl, 'qu\<times>'extra) cq_map\<close> is
  \<open>\<lambda>\<rho> c. if \<rho> \<ge> 0 then kf_map_outcome_inj (\<lambda>_. c) (kf_tensor_op_right \<rho>) else 0\<close>
  by (auto simp: bdd_above_def kf_map_outcome_inj_norm inj_on_def kf_norm_tensor_op_right)

lift_definition cq_map_partial_trace :: \<open>('cl, 'qu1\<times>'qu2, 'cl, 'qu1) cq_map\<close> is
  \<open>\<lambda>c. kf_map_outcome (\<lambda>_. c) (kraus_map_partial_trace (range ket))\<close>
  by (auto simp: bdd_above_def)
...
lift_definition cq_map_tensor_id_right :: \<open>('cl1, 'qu1, 'cl2, 'qu2) cq_map \<Rightarrow> ('cl1, 'qu1\<times>'extra, 'cl2, 'qu2\<times>'extra) cq_map\<close> is
  \<open>\<lambda>\<EE> c. kf_map_outcome fst (kf_tensor (\<EE> c) kf_id)\<close>
  apply (auto intro!: kraus_equivalent'_map_cong kf_tensor_cong'
      simp: bdd_above_def)
  by (smt (verit) kf_norm_tensor kraus_map_norm_id mult_cancel_left2)
...
lift_definition cq_map_infsum :: \<open>('a \<Rightarrow> ('cl1,'qu1,'cl2,'qu2) cq_map) \<Rightarrow> 'a set \<Rightarrow> ('cl1,'qu1,'a\<times>'cl2,'qu2) cq_map\<close> is
  \<open>\<lambda>\<EE> A. if \<forall>c. has_kraus_map_infsum (\<lambda>a. \<EE> a c) A \<and> bdd_above (range (\<lambda>c. kf_norm (kraus_map_infsum (\<lambda>a. \<EE> a c) A)))
...
lift_definition cq_map_classical :: \<open>('cl1 \<Rightarrow> 'cl2) \<Rightarrow> ('cl1, 'qu, 'cl2, 'qu) cq_map\<close> is
  \<open>\<lambda>f c. kf_map_outcome_inj (\<lambda>_. f c) kf_id\<close>
  by (auto simp: bdd_above_def kf_map_outcome_inj_norm inj_on_def)
