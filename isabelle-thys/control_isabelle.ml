...
fun executeMLInt seq ml =
  executeML ("Control_Isabelle.sendReply "^string_of_int seq^" [" ^ ml ^ "]")
...
fun handleLine seq line = case String.sub (line, 0) of
  ...
  | #"s" => store seq (E_Int (the (Int.fromString (String.extract (line, 1, NONE)))))
  ...
  | #"r" => retrieveInt seq (the (Int.fromString (String.extract (line, 1, NONE))))
    (* af x - Parses f,x as object#, f of type E_ExnExn, computes f x, stores the result, response 'seq ID' *)
  | #"a" => let val (f,x) = case String.fields (fn c => #" "=c) (String.extract (line, 1, NONE)) |> map (the o Int.fromString)
