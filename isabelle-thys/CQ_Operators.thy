...
(* TODO rename \<rightarrow> cq_operator *)
typedef ('cl,'qu) program_state = \<open>{\<rho> :: 'cl \<Rightarrow> ('qu ell2, 'qu ell2) trace_class. (\<forall>c. \<rho> c \<ge> 0) \<and> 
...
lift_definition fixed_cl_program_state :: \<open>'cl \<Rightarrow> ('qu ell2, 'qu ell2) trace_class \<Rightarrow> ('cl,'qu) program_state\<close> is
...
(* TODO rename \<rightarrow> cq_map *)
quotient_type ('cl,'qu) program_semantics = \<open>'cl \<Rightarrow> ('qu ell2, 'qu ell2, 'cl) kraus_family\<close> /
...
proof (rule part_equivpI)
  ...
  proof (rule sympI)
    ...
    then have eq1: \<open>kraus_equivalent' (\<EE> x) (\<FF> x)\<close> for x
      unfolding program_semantics_rel_def by blast
    ...
    have \<open>kraus_family_norm (\<EE> x) \<le> 1\<close> for x
      using asm unfolding program_semantics_rel_def by blast
    with eq3
...
lift_definition program_semantics_id :: \<open>('cl,'qu) program_semantics\<close> is 
  \<open>\<lambda>c. kraus_family_map_outcome (\<lambda>(). c) kraus_family_id\<close>
  by (simp add: program_semantics_rel_def)

lift_definition program_semantics_apply :: \<open>('cl,'qu) program_semantics \<Rightarrow> ('cl,'qu) program_state \<Rightarrow> ('cl,'qu) program_state\<close> is
...
proof (rename_tac \<EE> \<EE>' \<rho>, intro conjI allI ext)
  ...
  assume rel: \<open>program_semantics_rel \<EE> \<EE>'\<close>
...
lift_definition program_semantics_seq :: \<open>('cl,'qu) program_semantics \<Rightarrow> ('cl,'qu) program_semantics \<Rightarrow> ('cl,'qu) program_semantics\<close>
...
proof -
  ...
  assume asms: \<open>program_semantics_rel \<EE> \<EE>'\<close>  \<open>program_semantics_rel \<FF> \<FF>'\<close>
  ...
  have \<open>kraus_family_norm (kraus_family_comp_dependent \<FF> (\<EE> x)) \<le> 1 * kraus_family_norm (\<EE> x)\<close> for x
    ...
    using asms(2) unfolding program_semantics_rel_def by blast
...
lift_definition program_semantics_if :: \<open>('cl \<Rightarrow> bool) \<Rightarrow> ('cl,'qu) program_semantics \<Rightarrow> ('cl,'qu) program_semantics \<Rightarrow> ('cl,'qu) program_semantics\<close> is
  \<open>\<lambda>e p q c. if e c then p c else q c\<close>
  by (simp add: program_semantics_rel_def)

lift_definition program_semantics_quantum_op :: \<open>('cl \<Rightarrow> ('qu ell2, 'qu ell2, unit) kraus_family) \<Rightarrow> ('cl,'qu) program_semantics\<close> is
