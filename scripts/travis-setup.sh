...
mkdir -p ~/install
if ! [ -e ~/install/Isabelle2020 ]; then
  case "$TRAVIS_OS_NAME" in
    linux) curl https://isabelle.in.tum.de/website-Isabelle2020/dist/Isabelle2020_linux.tar.gz | tar -x -z -C ~/install;;
    osx) curl https://isabelle.in.tum.de/website-Isabelle2020/dist/Isabelle2020_macos.tar.gz | tar -x -z -C ~/install;;
...
if ! [ -e ~/install/afp-2020/thys ]; then
  curl https://www.isa-afp.org/release/afp-current.tar.gz |
      tar -x -z -C ~/install/afp-2020 --strip 1
fi

if ! timeout --kill-after 1m $TIME_LIMIT "$ISABELLE_HOME/bin/isabelle" build -o threads=8 -b -v -d isabelle-thys -d ~/install/afp-2020/thys QRHL-Prerequisites; then
